...
class source_data {
 public:
  source_data(source_type type, int id, uint32_t width = ICON_SIZE)
      : type(type), id(id), size(width), createTime(std::time(nullptr)) {}
  source_type type;
...
static std::unordered_map<std::string, int> NAME2ID{{u8"香蕉", 1},
                                                    {u8"快乐水", 17},
                                                    {u8"氧气瓶", 35},
                                                    {u8"好听", 46},
                                                    {u8"打Call", 12},
...
static void create_edge() {
  {
    b2Vec2 startpoint(scaled_width() * -1.f,
                      scaled_height() / 2.f - scaled_size() / 2.f);

    b2Vec2 endpoint(scaled_width() * 1.f,
                    scaled_height() / 2.f - scaled_size() / 2.f);
  ...
  {
    b2Vec2 startpoint(scaled_width() / -2.f - scaled_size() / 2.f,
                      scaled_height() * -1.f);

    b2Vec2 endpoint(scaled_width() / -2.f - scaled_size() / 2.f,
                    scaled_height() * 1.f);
...
static void add_gift(const std::string& user_name, const std::string& gift_name,
                     const int& count) {
  if (world) {
...
static uint32_t get_height(void* dat) { return height; }
...
static obs_properties_t* get_properties(void* data) {
  ...
  obs_properties_add_int(ppts, "width", obs_module_text("Width"), 144, 1920,
                         32);
  obs_properties_add_int(ppts, "height", obs_module_text("Height"), 144, 1080,
                         32);
  obs_properties_add_int(ppts, "expire", obs_module_text("Expire"), 10, 60, 5);
  obs_properties_add_path(ppts, "path", obs_module_text("Path"),
                          OBS_PATH_DIRECTORY, NULL, NULL);
  obs_properties_add_text(ppts, "uid", obs_module_text("UserId"),
                          OBS_TEXT_DEFAULT);
        ...
        switch (data->type) {
          ...
          case source_type::gift:
            if ((now - data->createTime) > expire) {
              ...
              const auto& x =
                  ((scaled_width() / 2.f) + pos.x) * MET2PIX + data->size;
              const auto& y =
                  ((scaled_height() / 2.f) + pos.y) * MET2PIX + data->size;
              ...
              gs_matrix_translate3f(data->size / -2.f, data->size / -2.f, 1.f);
              gs_draw_sprite(textures[data->id]->texture, 0, data->size,
                             data->size);
              gs_matrix_pop();
            } else {
