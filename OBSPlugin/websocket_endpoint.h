...
namespace AcFun {
	...
	public:
		...
		websocket_endpoint() {
			...
			thread = websocketpp::lib::make_shared<websocketpp::lib::thread>(&client::run, &endpoint);
		...
		bool connect(const uint64_t& uid, const std::function<void(const std::string&, const int&)>& addGift) {
			this->uid = uid;
	...
	private:
		...
		uint64_t uid = 0;
		std::function<void(const std::string&, const int&)> callback;
		client endpoint;
		...
		bool connect() {
			...
			conn->set_open_handler(websocketpp::lib::bind(&websocket_endpoint::on_open, this, &endpoint, websocketpp::lib::placeholders::_1));
			conn->set_fail_handler(websocketpp::lib::bind(&websocket_endpoint::on_fail, this, &endpoint, websocketpp::lib::placeholders::_1));
			conn->set_close_handler(websocketpp::lib::bind(&websocket_endpoint::on_close, this, &endpoint, websocketpp::lib::placeholders::_1));
			conn->set_message_handler(websocketpp::lib::bind(&websocket_endpoint::on_message, this, websocketpp::lib::placeholders::_1, websocketpp::lib::placeholders::_2));
		...
		void on_open(client* c, websocketpp::connection_hdl hdl) {
			char msg[64];
			const auto& len = sprintf_s(msg, "{\"cmd\":1,\"data\":{\"roomId\":%llu}}", uid);
		...
		void on_close(client* c, websocketpp::connection_hdl hdl) {
			const auto& conn = c->get_con_from_hdl(hdl);
			blog(LOG_INFO, "ws closed, code: %d(%s), reason: %s", conn->get_remote_close_code(), websocketpp::close::status::get_string(conn->get_remote_close_code()).c_str(), conn->get_remote_close_reason().c_str());
		...
		void on_message(websocketpp::connection_hdl hdl, client::message_ptr msg) {
			...
			if (data["cmd"].get<int>() == 3) {
				blog(LOG_INFO, "receive gift: %s, %d", data["data"]["giftName"].get<std::string>().c_str(), data["data"]["num"].get<int>());
				callback(data["data"]["giftName"].get<std::string>(), data["data"]["num"].get<int>());
			}
		}
	};
}
