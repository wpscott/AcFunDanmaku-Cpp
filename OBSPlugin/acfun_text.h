		...
		struct RGBText {
			...
			void update(obs_data_t* settings) {
				...
				if (new_face != font_face || new_font_size != font_size) {
					font_face = new_face;
					font_size = static_cast<FLOAT>(new_font_size);
				...
				gradient_direction = static_cast<float>(obs_data_get_int(settings, "direction"));
				step = static_cast<INT32>(obs_data_get_int(settings, "speed"));
			...
			void render_text() {
				if (!text.empty()) {
					UINT32 length = static_cast<UINT32>(wcslen(text.c_str()));
					float cx = 0, cy = 0;
					HRESULT hr = S_OK;
					...
					if (SUCCEEDED(hr)) {
						...
						cx = static_cast<float>(ceil(metrics.width));
						cy = static_cast<float>(ceil(metrics.height));
					...
					if (SUCCEEDED(hr)) {
						const float angle = static_cast<float>(DEG(atan(cy / cx)));
						if (gradient_direction <= angle || gradient_direction > 360.f - angle) {
							const float y = cx / 2.f * static_cast<float>(tan(RAD(gradient_direction)));
							gradient_x = cx;
							gradient_y = cy / 2.f - y;
							gradient2_x = 0.f;
							...
							const float x = cy / 2.f * static_cast<float>(tan(RAD(90.f - gradient_direction)));
							gradient_x = cx / 2.f + x;
							gradient_y = 0.f;
							...
							const float x = cy / 2.f * static_cast<float>(tan(RAD(270.f - gradient_direction)));
							gradient_x = cx / 2.f - x;
							gradient_y = cy;
							gradient2_x = cx / 2.f + x;
							gradient2_y = 0.f;
					...
					if (SUCCEEDED(hr)) {
						RECT rc;
						SetRect(&rc, 0, 0, static_cast<int>(cx), static_cast<int>(cy));

						obs_enter_graphics();
						if (!texture || width != rc.right || height != rc.bottom) {
							if (texture) {
								gs_texture_destroy(texture);
							}
							texture = gs_texture_create_gdi(rc.right, rc.bottom);
						}
			...
			static obs_properties_t* get_properties(void* data) {
				...
				obs_properties_add_int_slider(ppts, "speed", u8"速度", 1, 16, 1);
