...
struct Danmaku {
  ...
  ~Danmaku() {
    if (world) {
      ...
      while (node) {
        auto body = node;
        node = node->GetNext();
        delete (BodyData::body_data*)body->GetUserData();
  ...
  void initialize_direct_write() {
    ...
    if (SUCCEEDED(hr)) {
      hr = pWriteFactory->CreateTextFormat(L"Microsoft Yahei UI", NULL, weight,
    ...
    if (newWidth != width || newHeight != height) {
      ...
      if (world) {
        ...
        while (node) {
          auto body = node;
          node = body->GetNext();
          auto data = (BodyData::body_data*)body->GetUserData();
          if (data == nullptr) {
            world->DestroyBody(body);
          }
        }
  ...
  void render() {
    ...
    if (world) {
      if (show) {
        ...
        while (node) {
          auto body = node;
          const auto& data = (BodyData::body_data*)body->GetUserData();
            ...
            switch (data->type) {
              case BodyData::body_type::text:
                if ((now - data->createTime) > 30) {
                  ...
                  gs_matrix_translate3f(data->width / -2.f, data->height / -2.f,
                                        1.f);
