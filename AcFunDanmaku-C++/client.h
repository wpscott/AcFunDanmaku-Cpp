...
class Client {
 public:
    ...
    return client.request(request)
        .then([&](http_response response) {
          if (!isOK(response)) {
            ...
            if (didCookie.empty() || did.empty()) {
              for (auto& [key, value] : headers) {
                if (key == SetCookieHeader &&
                    value.find(_did_key) != string_t::npos) {
                  didCookie = value;
                  did = didCookie.substr(
                      _did_key.length(),
                      didCookie.find_first_of(';') - _did_key.length());
                  break;
                }
              }
            }
        ...
        .then([&](http_response response) {
          if (!isOK(response)) {
            ...
            uri_stream << U("/rest/zt/live/web/"
                       ...
                       << U("&userId=") << userId << U("&did=") << did << U("&")
                       << VisitorToken << U("=") << serviceToken;
        ...
        .then([&](http_response response) {
          if (!isOK(response)) {
            ...
            if (json[U("result")].as_number().to_int32() != 1) {
              ...
              auto& _tickets =
                  json[U("data")][U("availableTickets")].as_array();
              for (auto& ticket : _tickets) {
                tickets.push_back(
                    conversions::to_utf8string(ticket.as_string()));
              }
              enterRoomAttach =
                  json[U("data")][U("enterRoomAttach")].as_string();
  ...
  const Gift* getGift(int64_t giftId) {
    const auto& it = giftList.find(giftId);
    if (it == giftList.end()) {
      return nullptr;
    } else {
      return &it->second;
  ...
  void set_handler(const std::function<void(const std::string&,
  ...
  pplx::task<bool> start() {
    auto request =
        client_request(userId, conversions::to_utf8string(serviceToken),
                       conversions::from_base64(securityKey),
                       conversions::to_utf8string(liveId),
                       conversions::to_utf8string(enterRoomAttach), tickets);
    ...
    try {
      ...
      client.connect(WS_HOST).wait();

      client.send(request.RegisterRequest()).wait();

      while (running) {
        try {
          const auto msg = client.receive().get();
          const auto& [header, down] = ClientUtils::Decode(
              msg, request.getSecurityKey(), request.getSessionKey());
          ...
          if (command == Command::GLOBAL_COMMAND) {
            ...
            if (cmdack == GlobalCommand::ENTER_ROOM_ACK) {
              ...
              timer = new boost::asio::deadline_timer(
                  ios, boost::posix_time::milliseconds(heartbeatinterval));
              timer->async_wait(hbhandler);
            ...
            if (message.compressiontype() ==
                AcFunDanmu::ZtLiveScMessage::GZIP) {
            ...
            if (msgType == PushMessage::ACTION_SIGNAL ||
              ...
              if (type == AcFunDanmu::ZtLiveScStatusChanged::LIVE_CLOSED ||
                  type == AcFunDanmu::ZtLiveScStatusChanged::LIVE_BANNED) {
                stop();
                break;
              }
...
string_t AcFunDanmu::Client::didCookie = {};
string_t AcFunDanmu::Client::did = {};
